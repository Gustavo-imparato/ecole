...
public class InstrutorRepository implements Repository<Instrutor, Long>{
    ...
    public Instrutor findById(Long id) {
        return instrutors.stream().filter(i -> i.getId().equals(id )).findFirst().orElse(null);
    }

    @Override
    public List<Instrutor> findByName(String texto) {
        return instrutors.stream().filter(i -> i.getNome().toLowerCase().contains(texto.toLowerCase() )).toList();
    }
